...
# 2018.9.6 10:00 7.1 访问串口使用互斥锁，崩溃问题解决；手动模式完成；单片机开关转至7.2
ser = serial.Serial('com8', baudrate=9600, bytesize=8, timeout=0.1)  # 连接串口1,RS232
# '/dev/ttyAMA3'
db = pymysql.connect("10.0.0.112", "root", "password", "green")  # 连接数据库
...
def f_read_flow_meter():
    ...
    if len(read_flow_meter_raw) >= 17:
        ...
        if (cal_received_crc_h == received_flow_meter_crc_raw_h
            ...
            try:
                sqlinsert1 = "insert into data values(default,'%d','%s','%s','%.1f','%s')" % (
                    9, 'flow_meter_total', t, flow_meter_total, 'z')
                sqlinsert2 = "insert into data values(default,'%d','%s','%s','%.1f','%s')" % (
                    9, 'flow_meter_instant', t, flow_meter_instant, 'z')
...
def camera():
    ...
    # except:
    #     pass
    # 执行拍照，保存；文件名为拍照时间；linux shell streamer -h获取帮助信息
        ...
        try:
            ...
            for water in water_switch_addr:  # 水泵开关循环
                ...
                if water == 91 or water == 92:  # 打开下蓄水池水泵
                    water_pump_down = 1
            # 继电器开关开循环
            for water1 in water_switch_addr:  # 继电器开关循环
                if water1 == 81:
                    i = 1
                    while True:
                        lock.acquire()
            ...
            if water_tapwater == 1 and water_flag != 6:
                ...
                print('检测流量计流量，如果流量计过小，打开89号继电器（加压水泵）')
                # time.sleep(5)  # 等待水流稳定，测量流速
                # sleep = 1
                flow_meter_instant2 = th_read_flow_meter()  # 调用流量计函数，不写入数据库的
                print(flow_meter_instant2)
                # 自来水压力过小，打开加压水泵.............（检测的前几次流量不做参考）
                if flow_meter_instant2 <= 0.1:  # 自来水压力过小，打开加压水泵
                    i = 1
                    while True:
                        lock.acquire()
                        ser.write([0xff, 0xff, 89, 1, 88])
...
def th_sensor():
    ...
    while True:
        ...
        for select_addr in sensor_addr:
            ...
            db.commit()
        time.sleep(1)  # 循环读取数据库的时间,要求>1min一测
        # except:
        #     pass
