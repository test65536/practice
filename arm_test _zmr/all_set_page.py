...
class All_device_set(QWidget):
    ...
    def initUI(self):
        ...
        btn_exit.setGeometry(400,500,200,30)
    ...
    #
    #     else:
    #         sql_ = "update command1 set manual = '%s',manswitch = '%s'"%('Y',0)
    #         self.cursor.execute(sql_)
    #         self.db.commit()
    #         print('关闭电磁阀')
    ...
    def update_the_irrigation_time(self):
        ...
        sql_man = "select manual,name from command1 where id < 9"
        self.cursor.execute(sql_man)
        if_man = self.cursor.fetchall()
            ...
            for i in if_man:
                if i[0] == 'Y':
                    sql_ = "update command1 set manual = '%s',type = '%s',timing11 = '%s',timing12 = '%s',timing21='%s',timing22='%s',manswitch = '%s',switch1 = '%s' where name = '%s'" \
                           % ('N', 'A', self.start_time_1, self.end_time_1, self.start_time_2, self.end_time_2, 0, 0,i[1])
                    self.cursor.execute(sql_)
                    self.db.commit()
                else:
                    sql_ = "update command1 set manual = '%s',type = '%s',timing11 = '%s',timing12 = '%s',timing21='%s',timing22='%s',manswitch = '%s' where name = '%s'" \
                           % ('N', 'A', self.start_time_1, self.end_time_1, self.start_time_2, self.end_time_2, 0,i[1])
                    self.cursor.execute(sql_)
                    self.db.commit()
    ...
    def update_the_automatic_time(self):
        ...
        print('时间段1：%s - %s' % (self.start_time_1, self.end_time_1))
        print('时间段2：%s - %s' % (self.start_time_2, self.end_time_2))
        sql_man = "select manual,name from command1"
        self.cursor.execute(sql_man)
        if_man = self.cursor.fetchall()
        print('之前是否是手动模式:', if_man)
        if self.start_time_1 > self.end_time_1:
            ...
            for i in if_man:
                if i[0] == 'Y':
                    sql_ = "update command1 set manual = '%s',type = '%s',timing11 = '%s',timing12 = '%s',timing21='%s',timing22='%s',manswitch = '%s',switch1 = '%s' where name = '%s'" \
                           % ('N', 'B', self.start_time_1, self.end_time_1, self.start_time_2, self.end_time_2, 0, 0,i[1])
                    self.cursor.execute(sql_)
                    self.db.commit()
                else:
                    sql_ = "update command1 set manual = '%s',type = '%s',timing11 = '%s',timing12 = '%s',timing21='%s',timing22='%s',manswitch = '%s' where name = '%s'" \
                           % ('N', 'B', self.start_time_1, self.end_time_1, self.start_time_2, self.end_time_2, 0,i[1])
                    self.cursor.execute(sql_)
                    self.db.commit()
