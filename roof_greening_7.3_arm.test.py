...
ser = serial.Serial('com8', baudrate=9600, bytesize=8, timeout=0.1)  # 连接串口1,RS232
# '/dev/ttyAMA3'
db = pymysql.connect("10.0.0.111", "root", "password", "green")  # 连接数据库
...
def f_read_flow_meter():
    ...
    if len(read_flow_meter_raw) >= 17:
        ...
        if (cal_received_crc_h == received_flow_meter_crc_raw_h
            ...
            try:
                sqlinsert1 = "insert into data values(default,'%d','%s','%s','%.1f','%s')" % (
                    9, 'flow_meter_tapwater_total', t, flow_meter_total, 'z')
                sqlinsert2 = "insert into data values(default,'%d','%s','%s','%.1f','%s')" % (
                    9, 'flow_meter_tapwater_instant', t, flow_meter_instant, 'z')
                db.ping(reconnect=True)
...
def camera_windows():
    ...
    print('----拍照成功，即将将文件名-%s-写入数据库------！' % picture_time)
    sqlinsert10 = "insert into pictures values(default,'%s')" % (picture_time + '.png')
    db.ping(reconnect=True)
    cursor.execute(sqlinsert10)  # 执行插入数据操作
    # cursor.execute("insert into pictures values(default,'%s')" % (time_picture + '.jpeg'))
    db.commit()
...
def th_sensor():
    global rainfall_times_flags  # 降雨量测量次数标志位
    # lock.acquire()
    db = pymysql.connect("10.0.0.111", "root", "password", "green")  # 连接数据库
    db.autocommit(True)
    cursor = db.cursor()  # 使用cursor()方法获取操作游标
    ...
    for select_addr in sensor_addr:
        ...
        # 调用读取降雨量函数,并接收函数返回值
        if select_addr == 5:
            rainfall_times_flags = rainfall_times_flags + 1
            print(rainfall_times_flags)
            if rainfall_times_flags % 2 == 0:
                print('等待1min...')
                thread1 = threading.Thread(target=th_rainfall)  # 建立降雨量线程
                thread1.start()  # 开始降雨量线程，以下代码继续执行，降雨量测量完毕后输出
                time.sleep(1)
                rainfall_times_flags = 0
        ...
        if select_addr == 6:
            while True:
                if i > 2:
                    ...
                    print('异常数据即将写入数据库...')
                    try:
                        sqlinsert18 = "insert into data values(default,'%d','%s','%s','%.1f','%s')" % (
                            6, 'humidity', t, 888, 'y')  # 3个异常都写入
                        sqlinsert19 = "insert into data values(default,'%d','%s','%s','%.1f','%s')" % (
                            6, 'temperature', t, 888, 'y')  # 3个异常都写入
                        sqlinsert20 = "insert into data values(default,'%d','%s','%s','%.1f','%s')" % (
                            6, 'light', t, 888, 'y')  # 3个异常都写入
                        db.ping(reconnect=True)
                        cursor.execute(sqlinsert18)  # 执行插入humidity异常数据
                        db.commit()
                        db.ping(reconnect=True)
                        cursor.execute(sqlinsert19)  # 执行插入temperature异常数据
                        db.commit()
                        db.ping(reconnect=True)
                        cursor.execute(sqlinsert20)  # 执行插入light异常数据
                        db.commit()
                    except:
                        pass
                    print('*' * 40)
        ...
        if select_addr == 9:
            while True:
                if i > 3:
                    ...
                    print('异常数据即将写入数据库...')
                    try:
                        sqlinserta = "insert into data values(default,'%d','%s','%s','%.1f','%s')" % (
                            9, 'flow_meter_tapwater_total', t, 888, 'y')
                        sqlinsertb = "insert into data values(default,'%d','%s','%s','%.1f','%s')" % (
                            9, 'flow_meter_tapwater_instant', t, 888, 'y')
                        db.ping(reconnect=True)
                        cursor.execute(sqlinserta)  # 执行插入异常数据flow_meter_tapwater_total
                        db.commit()
                        db.ping(reconnect=True)
                        cursor.execute(sqlinsertb)  # 执行插入异常数据flow_meter_tapwater_instant
                        db.commit()
                    except:
                        pass
                    print('*' * 40)
...
if __name__ == '__main__':
    ...
    rainfall_times_flags = 0  # 降雨量次数，标志位
    while True:
        a = time.time()
        water_switch()
        th_sensor()
